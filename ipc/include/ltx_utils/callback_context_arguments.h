...
struct __declspec(uuid("{07A41DD0-CF89-48b3-A02F-7CA7D4B1AAF4}")) i_wbs_handler:IUnknown
{
	...
	virtual HRESULT STDMETHODCALLTYPE send_frame(int fin,void* pdata,INT64* psize)=0;
...
{		
	...
	struct ptr_t
	{
		...
		ptr_t(i_mm_region_ptr * _p, int64_t _ByteOffset = 0, int64_t _ByteLength = sizeof(T))
			:p(NULL), prelease(NULL), ByteLength(_ByteLength), ptr(0), hr(E_POINTER) {

			p=init(_p, _ByteOffset, _ByteLength);
		}


		ptr_t(VARIANT v, int64_t _ByteOffset =-1, int64_t _ByteLength =-1)
			:p(NULL), prelease(NULL), ByteLength(_ByteLength), ptr(0), hr(E_POINTER) {

			VARTYPE vt = v.vt;
			IUnknown *pu = v.punkVal;
			if (!pu)
				return;
			if ((vt == VT_DISPATCH) || (vt == VT_UNKNOWN)) {
				if (SUCCEEDED(hr = pu->QueryInterface(__uuidof(i_mm_region_ptr), (void**)&p))) {
					prelease= init(p, _ByteOffset, _ByteLength);
				}
		...
		/*
		template <class V>
		ptr_t(V&& v, int64_t _ByteOffset = -1, int64_t _ByteLength = -1) {
			::new(this) ptr_t(VARIANT(v), _ByteOffset , _ByteLength );
		}
		*/
		inline i_mm_region_ptr *init(i_mm_region_ptr * p, int64_t _ByteOffset, int64_t _ByteLength)			
		{
		...
		~ptr_t()
		{
			//SUCCEEDED(hr)||(p->DecommitRegionPtr(ptr));
			(p) && (p->DecommitRegionPtr(ptr));
			(prelease) && (prelease->Release());
		}
		...
		inline i_mm_region_ptr* intf() {
			return p;
		}

		i_mm_region_ptr * p,*prelease;
		int64_t ByteLength;
		int64_t capacity;
		T* ptr;
