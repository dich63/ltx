...
namespace ipc_utils {
	...
	struct com_exception_helper_t
	{
		...
		template <class O>
		static HRESULT check_error(HRESULT hr, O* p) { return HRESULT_FROM_WIN32(hr); }
	...
	struct com_exception_helper_t<com_exception>
	{
		...
		static HRESULT check_error(HRESULT hr) {
			...
			return (hr==S_OK)? S_OK :check_error(hr, (ltx_helper::exceptinfo_t*)NULL);
		}

		template <class CH>
		static HRESULT check_error(HRESULT hr, CH* pmsg) {

			if (FAILED(hr)) {

				ltx_helper::exceptinfo_t ei;
				ei.bstrSource= bstr_t("asyn JScript").Detach();
				ei.bstrDescription = bstr_t(pmsg).Detach();
				ei.scode = hr;

				return check_error(hr, &ei);
			}

			return hr;
		}
		
		static HRESULT check_error(HRESULT hr,ltx_helper::exceptinfo_t* pei){ 

			  if(FAILED(hr)){

				  throw com_exception(hr,(pei) ? pei->ErrorInfo(hr, true):0,true);
	...
	struct dispcaller_t:invoker_base_t<dispcaller_t<ExcetionClass>,variant_t,variant_t,VARIANT>
	{
		...
		HRESULT reset(bstr_t moniker_string, DISPID id = DISPID_VALUE) {

			HRESULT hr;
			comdef.init();

			smart_ptr_t<IDispatch> dispnew;			
			if (SUCCEEDED(hr = dispnew.CoGetObject(moniker_string))) {
				hr= reset(dispnew,id);
			}
			exception_helper_t::check_error(hr);
	...
	inline com_dispcaller_t& terminal_js(const com_dispcaller_t& j = com_dispcaller_t("ltx.bind:script:")) {
		com_dispcaller_t& jr = const_cast<com_dispcaller_t&>(j);
		jr("require('rgs').term()");
		return jr;
	};	
	inline com_dispcaller_t& terminal_js(bstr_t mon) {
		return terminal_js(com_dispcaller_t("ltx.bind:script:"));
	}
