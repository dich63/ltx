...
#include "dispatch_call_helpers.h"
	...
	template <class F>
	const char* type2str() {
		return typecase_t<double>().name();
	}
	template <class F>
	const char* type2str(F f) {
		return typecase_t<double>().name();
	}

	struct buffer_factory_t {

		buffer_factory_t(bstr_t sc = "ltx.bind:script:", bstr_t t = "require('tensor').createSparseBuffer")
			:jc(sc),
			createBuffer(jc(t)) {};
	...
	template <class F>
	variant_t createBuffer(size_t size) {

		bstr_t type = typecase_t<F>::name();

		ipc_utils::com_dispcaller_t jc("ltx.bind:script:");

		ipc_utils::com_dispcaller_t create = jc("require('tensor').createBuffer");
		return create(double(size), type);
	}


	template <class F>
	variant_t data2buffer(size_t size, F*data, buffer_factory_t& factory) {
	...
	size_t buffer_set(caller_t& setter, bstr_t name,size_t size, F*data, buffer_factory_t& factory) {
	...
	template <class F,class caller_t= ipc_utils::com_dispcaller_t>
	size_t vector_set(caller_t& setter, bstr_t name,std::vector<F>& v, buffer_factory_t& factory ) {		
